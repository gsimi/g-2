...
float channel::time(bool negative, int bkgmax, int sigmax ){
   ...
   for (int i=sigmin;i<sigmax;i++) {
     if ((sgn*calibval[i]-pedestal)>0.4*(max-pedestal)) {
       t4=(t[i-1]+t[i])/2; break;
       iprev=i;
     }
   }
   for (int i=iprev;i<sigmax;i++) {
     if ((sgn*calibval[i]-pedestal)>0.6*(max-pedestal)) {
       t6=(t[i-1]+t[i])/2; 
       break;
     }
   }
...
TH1F* pulseheight(const char * fdata="data/wave.01_1.txt", 
		  const char* fpedestals="pedestals/pedestal.01_1.txt", algorithm alg=integral2, int tmin=0, int tmax=1024){
