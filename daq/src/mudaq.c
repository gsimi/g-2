...
void
caenvmeerrorcheck(CVErrorCodes  ret){
    switch (ret)
      {
...
Restart:
    ...
    uint32_t discBaseAddress=0x00BB0000;
    if (ret = CAENComm_OpenDevice((CAENComm_ConnectionType)WDcfg.LinkType,WDcfg.LinkNum,WDcfg.ConetNode,discBaseAddress,&dischandle)) {
      ErrCode = ERR_DGZ_OPEN;
      goto QuitProgram;
    }
    uint16_t discdata=0;
    uint32_t discregister=0xFC;
    CAENComm_Read16(dischandle, discregister, &discdata );
    printf( "discdata in FC %x\n",discdata);

    discregister=0xFA;
    CAENComm_Read16(dischandle, discregister, &discdata );
    printf( "discdata in FA %x\n",discdata);
    ...
    for (discch=0;discch<16;discch++){
      discregister=discch*2;
      CAENComm_Write16(dischandle, discregister, discth);
    }

    //enable mask: enable all channels
    discregister=0x4a;
    CAENComm_Write16(dischandle, discregister, 0xFF);

    //set the pulse width 40ns
    discregister=0x40;
    CAENComm_Write16(dischandle, discregister, 0xFF);
    discregister=0x42;
    CAENComm_Write16(dischandle, discregister, 0xFF);
