...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  uint32_t enabled_channels[2] = {   0x1e00,  0xc000 }; // 0 --> trg   1 --> signal
...
/*
  ...
  */
  ...
  int itrig;
  for (itrig=1; itrig<3; itrig++){			
  // *** registri minterm per i vari trigger (5004,5008,500c)
    jdiscregister=0x5000+trigger_offset[itrig]; 
    uint32_t mask_channel=0xffff & ~(enabled_channels[itrig-1]);	// *** esegue l'AND bit a bit
    
  // ***  std::cout<<"Prova \t itrig : " << itrig << " \t, canali abilitati: " << enabled_channels[itrig] << "\n" ;
    printf(" \n \nProva \t itrig:  %d\t, canali abilitati: 0x%x\t",itrig ,enabled_channels[itrig-1]);   
