...
*/
	...
	if (ch == chA) 
	  ...
	  treeEvt->SetBranchAddress("WaveformsB",&sample[0]);


	Long64_t nEvt = treeEvt->GetEntries();
	float maximum = 0.0;
	float minimum = 0.0;
	float range = ch == chA ? chSet1.range  : chSet2.range;
	cout<<"range  = "<<range<<endl;
	// spettro in energia
	float xmin= negative? adc_to_mv(sampSet.max_adc_value,range,-1*sampSet.max_adc_value) : 0 ; 
	float xmax = negative? 0 : adc_to_mv(sampSet.max_adc_value,range,sampSet.max_adc_value) ;
	TH1F* spectrumMaximum = new TH1F( "hMax", "Maxima Distribution Spectrum", nbin, xmin,xmax );
//////////////////////////////////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
	for (Long64_t index=0; index<nEvt; index++) 
	{
		...
		{
			float value =  adc_to_mv(sample[ii],range,sampSet.max_adc_value);
