 ...
 *
 *	The functions provided in this source file and it's corresponding dll
 *	are to be used for Fast Streaming Data capture for the following 
 *	devices:
 *
...
extern void PREF0 PREF1 SetAggregateBuffer(int16_t handle, int16_t channel, int16_t * bufferMax, int16_t * bufferMin, uint32_t bufferSize)
{
    ...
    /*_buffers[index] = _buffers[index + 1] = NULL;
    _bufferSizes[index] = _bufferSizes[index + 1] = bufferSize;

    if(bufferMax != NULL) _buffers[index] = bufferMax;
    if(bufferMin != NULL) _buffers[index + 1] = bufferMin;*/
