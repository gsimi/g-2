		...
		if(g_digitalPortCount > 0)
		{
			...
			for(digitalPort = (int16_t) PS2000A_WRAP_DIGITAL_PORT0; digitalPort < g_digitalPortCount; digitalPort++)
			{
				if(g_enabledDigitalPorts[digitalPort])
				{
					...
					{
						// Max digital buffers
						if (wrapBufferInfo->appDigiBuffers[channel * 2]  && wrapBufferInfo->driverDigiBuffers[channel * 2])
						{
						...
						// Min digital buffers
						if (wrapBufferInfo->appDigiBuffers[channel * 2 + 1]  && wrapBufferInfo->driverDigiBuffers[channel * 2 + 1])
						{
							// Min digital buffers
...
extern PICO_STATUS PREF0 PREF1 RunBlock(int16_t handle, int32_t preTriggerSamples, int32_t postTriggerSamples,
            uint32_t timebase, uint16_t segmentIndex)
{
