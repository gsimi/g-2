...
'   17-03-2015  Revision 3      HSM         Block data collection uses delegate
'
'========================================================================================
...
Module PS4000
    ...
    Const PS4262_MAX_VALUE = 32767          ' MAX ADC value used when converting between ADC values and mV for the PicoScope 4262
    Const PS4000_MAX_VALUE = 32764          ' MAX ADC value used when converting between ADC values and mV for all other PicoScope 4000 Series models
    ...
    Declare Function ps4000GetTimebase Lib "ps4000.dll" (ByVal handle As Short, ByVal timebase As UInteger, ByVal noSamples As Integer, ByRef timeInterval As Integer, ByVal oversample As Short, _
                                                         ByRef maxSamples As Integer, ByVal segment As UShort) As UInteger

    Declare Function ps4000RunBlock Lib "ps4000.dll" (ByVal handle As Short, ByVal noPreTriggerSamples As Integer, ByVal noPostTriggerSamples As Integer, ByVal timebase As UInteger, _
    ...
    Declare Function ps4000GetValues Lib "ps4000.dll" (ByVal handle As Short, ByVal startIndex As UInteger, ByRef numSamples As UInteger, ByVal downSampleRatio As UInteger, ByVal downsampleRatioMode As Short, _
    ...
    Declare Function ps4000SetSimpleTrigger Lib "ps4000.dll" (ByVal handle As Short, ByVal enable As Short, ByVal source As Channel, ByVal threshold As Short, ByVal direction As LevelTrig, ByVal delay As UInteger, _
                                                              ByVal autoTriggerMs As Short) As UInteger

    Declare Function ps4000SetTriggerChannelDirections Lib "ps4000.dll" (ByVal handle As Short, ByVal channelA As LevelTrig, ByVal channelB As LevelTrig, ByVal channelC As LevelTrig, ByVal channelD As LevelTrig, _
                                                                         ByVal ext As LevelTrig, ByVal aux As LevelTrig) As UInteger

    Declare Function SetTriggerProperties Lib "ps4000Wrap.dll" (ByVal handle As Short, ByRef triggerChannelPropertiesArray As Integer, ByVal nProperties As Short, ByVal autoTrig As Integer) As UInteger
    Declare Function SetTriggerConditions Lib "ps4000Wrap.dll" (ByVal handle As Short, ByRef ConditionsArray As Integer, ByVal nConditions As Short) As UInteger
    ...
    Sub GetDeviceInfo(ByRef UnitModel As UNIT_MODEL)
        ...
        For i = 0 To 6
            str = "            "
            ps4000GetUnitInfo(UnitModel.handle, str, 255, requiredSize, i)
    ...
    Sub BlockDataHandler(ByRef UnitModel As UNIT_MODEL)
        ...
        For chan = 0 To UnitModel.channelCount - 1              ' Allocate one row of the array per channel
            status = ps4000SetDataBuffer(UnitModel.handle, chan, buffers(chan, 0), totalSamples)
        Next
        ...
        If keypress = False Then                       ' Data must have been collected if keypress is False
            ...
            status = ps4000GetValues(UnitModel.handle, 0, totalSamplesCollected, 1, 0, 0, 0) ' handle, startIndex, *NoSamples, downSampleRatio, 0=RATIO_MODE_NONE, segmentIndex, overflow
            ...
            If mvConvert = True Then                                        ' Convert values to mV
                ...
                For i = 0 To 100 Step i + 1                                 ' show first 100 values
                    ...
                    For chan = 0 To UnitModel.channelCount - 1              ' On all channels
                        ...
                        If UnitModel.channelSettings(chan).enabled Then
                            Console.Write(CShort(adc_to_mv(buffers(chan, i), UnitModel.channelSettings(chan).range, UnitModel.maxADCValue)) & vbTab)
                        End If
    ...
    Function TriggerPropertiesSetup(ByRef UnitModel As UNIT_MODEL, ByRef TrigSettings As TRIG_SETTINGS)
        ...
        For i = 0 To TrigSettings.NumberTrigs - 1 Step 1
            ...
            triggerChannelPropertiesArray((j * 6) + 0) = trigADC          ' Upper Threshold
            triggerChannelPropertiesArray((j * 6) + 1) = hystADC          ' Upper Hysteresis
            triggerChannelPropertiesArray((j * 6) + 2) = 0                ' Lower Threshold
            triggerChannelPropertiesArray((j * 6) + 3) = 0                ' Lower Hysteresis
        ...
        ' Use the Wrapper code's SetTriggerProperties function to create the parameters used in the 'ps4000SetTriggerChannelProperties' call
        status = SetTriggerProperties(UnitModel.handle, triggerChannelPropertiesArray(0), TrigSettings.NumberTrigs, 0)    'handle, propertiesArray, Num. channels, AutoTrigger_Time)
    ...
    Function TriggerDirectionsSetup(ByVal UnitModel As UNIT_MODEL)
        ...
        status = ps4000SetTriggerChannelDirections(UnitModel.handle, LevelTrig.RISING, LevelTrig.RISING_OR_FALLING, LevelTrig.RISING_OR_FALLING, _
